ma_dp_fc_compare = function(a, b, filter_detected = F, file_prefix = NA, p_thresh = 2){
  ...
  # keep = grepl(pattern = "unique_peak1", ma_res[, 4])
  # ma_res[keep, 4] = paste(l1, "higher than", l2, "for", m)
  # 
  # keep = grepl(pattern = "unique_peak2", ma_res[, 4])
  # ma_res[keep, 4] = paste(l2, "higher than", l1, "for", m)
  # 
  # keep = grepl(pattern = "common_peak1", ma_res[, 4])
  # ma_res[keep, 4] = paste(l1, "shared with", l2, "for", m)
  # 
  # keep = grepl(pattern = "common_peak2", ma_res[, 4])
  # ma_res[keep, 4] = paste(l2, "shared with", l1, "for", m)
  ...
  # keep = grepl(pattern = "common", diffpeak_res[, 4])
  # diffpeak_res[keep, 4] = paste(l1, "same as", l2, "for", m)
  # 
  # keep = grepl(pattern = "cond1", diffpeak_res[, 4])
  # diffpeak_res[keep, 4] = paste(l1, "higher than", l2, "for", m)
  # 
  # keep = grepl(pattern = "cond2", diffpeak_res[, 4])
  # diffpeak_res[keep, 4] = paste(l1, "lower than", l2, "for", m)
  ...
  plot_2lists_wrap = function(list_a, list_b, scale = NA, plot_count = 0, plot_title = 'TITLE'){#, scale_a = NA, scale_b = NA){
    if(length(scale) == 1 && is.na(scale)){#i'm assuming both scales will be set or neither
      scale = rep(1, nrow(my_fe))
      names(scale) = rownames(my_fe)
    }
    
    if(!is.na(file_prefix)) png(paste0(file_prefix, '_', plot_count, '.png'), width = 6, height = 10, res = 150, units = 'in')
    plot_2lists(my_fe, list_a, list_b, a, b, scale, plot_title)
    if(!is.na(file_prefix)) dev.off()
  }
